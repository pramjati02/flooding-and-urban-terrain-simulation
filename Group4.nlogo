...
;; patches-own is taken from the Terrain Generator model v.0.1.
;; (Andreas Angourakis, 2018)
patches-own
[
  land
  elevation
...
to setup
  ...
  ask n-of 1 patches [ set pcolor grey ]
  reset-ticks
  set initial-size count patches
  set city-size 0

end

;; to go is (partly) taken from the NetLogo Grand Canyon model.
;; (Uri Wilensky, 2006)
to go

  if raining and not any? turtles-on patch mouse-xcor mouse-ycor
  [
    create-raindrops rain-amount
  ...
  if ticks mod citybuildspeed = 0 [
  ask patches with [ pcolor = grey ] [
      let target one-of neighbors4 with [elevation > sealevel]
  ...
  set total-land count patches with [elevation > sealevel]
  set flooded-patches count patches with [ elevation > sealevel and count raindrops-here > 1]



  if ticks mod killrainafterxticks = 0 [
  ask turtles with [pcolor != grey] [ die]
  ]
  set city-size count patches with [pcolor = grey]
  set urbanisation (city-size / initial-size) * 100
  ask raindrops [ flow ]
...
;; setLandform is taken from the Terrain Generator model v.0.1.
;; (Andreas Angourakis, 2018)
to setLandform
...
;; paintPatches is taken from the Terrain Generator model v.0.1.
;; (Andreas Angourakis, 2018)
to paintPatches
...
;; setup-patch-coordinates-labels [ XcoordPosition YcoordPosition ] is taken from the Terrain Generator model v.0.1.
;; (Andreas Angourakis, 2018)
...
;; setup-transect is taken from the Terrain Generator model v.0.1.
;; (Andreas Angourakis, 2018)
to setup-transect
...
;; plot-horizontal-transect is taken from the Terrain Generator model v.0.1.
;; (Andreas Angourakis, 2018)
...
;; plot-vertical-transect is taken from the Terrain Generator model v.0.1.
;; (Andreas Angourakis, 2018)
...
;; to flow is taken from the NetLogo Grand Canyon model.
;; (Uri Wilensky, 2006)
...
"" ""
PENS
"pen-1" 1.0 0 -11221820 true "" "plot count turtles"
...
"" ""
PENS
"default" 1.0 0 -9276814 true "" "plot urbanisation"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot flooded-patches"
